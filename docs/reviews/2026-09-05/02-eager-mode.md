...
검증 방법: `curl -sL raw.githubusercontent.com/pytorch/pytorch/<tag>/<path>`로 v2.13.0(비교 대상 v2.5.0/v2.2.0/main) 소스를 내려받아 `sed -n`/`grep -n`으로 인용된 줄·시그니처를 직접 대조. GitHub 이슈는 `api.github.com`으로 제목·본문 확인. NVIDIA·RBLN 문서는 `curl`로 HTML을 받아 텍스트 추출 후 대조(WebFetch 대신 curl 사용, 모두 1회 200 응답, 재시도 불필요).

| # | 인용 위치 | 출처 | 접근 | 판정 | 근거 (인용/줄 번호) |
|---|---|---|---|---|---|
| 1 | 병합 TL;DR, Fable 확인목록(157-159행) | v2.13.0 `Dispatcher.h:833` | 200 | 지지 | 833행이 정확히 `inline Return Dispatcher::redispatch(` 정의 시작 |
| 2 | Fable A4-3(135행), Codex A4-3(227행) | v2.13.0 `Dispatcher.h` `callWithDispatchKeySlowPath` (708행) | 200 | 지지 | 708행 `inline Return Dispatcher::callWithDispatchKeySlowPath(`; 785-793행에서 `kernel = op.lookup(dispatchKeySet)` 후 `step_callbacks.has_value() && isObserved()`일 때만 707행 함수 호출 — RecordFunction/profiler 경로라는 설명과 정확히 일치 |
| 3 | Fable A4-4(136행), Codex A4-2(225행) | v2.13.0 `Dispatcher.h:777-779` | 200 | 지지 | 777-779행 `auto dispatchKeySet = op.operatorDef_->op.dispatchKeyExtractor().template getDispatchKeySetUnboxed<Args...>(args...);` — `Dispatcher::call` 내부에서 계산됨을 그대로 확인 |
| 4 | Codex A4-4(229행) | v2.13.0 `Dispatcher.h` (일반 링크) | 200 | 지지 | `Dispatcher::call`은 kernel 찾아 즉시 호출만 하고 "다음 key로 자동 반복"하는 루프가 없음(774-829행); redispatch는 별도 함수(833행)로 kernel이 명시적으로 호출해야 함 |
| 5 | Fable A4-1(133행) | v2.13.0/v2.5.0/v2.2.0 `CUDABlas.cpp` `gemm<float>`(1584)→`gemm_internal<float>`(1391)→`gemm_internal_cublas<float>`(1133) | 200(3개) | 지지 | v2.13.0: 1584-1585행 `gemm<float>`이 else 분기에서 1391행 `gemm_internal<float>` 호출, 그 안(≈1400행 내)에서 1133행 `gemm_internal_cublas<float>`(→`cublasSgemm`) 호출 — 인용한 세 줄 정확히 일치. `grep -c gemm_internal`: v2.13.0=122, v2.5.0=74, v2.2.0=0 |
| 6 | Fable A4-5(137행) | v2.13.0 `DispatchKeyExtractor.h:186-198` | 200 | 지지 | 186-198행이 인용 코드와 토씨까지 일치(`requiresBitsetPerBackend_`, `tls_local_dispatch_key_set()`, `nonFallthroughKeysPerBackend_[backend_idx]`) |
| 7 | Fable A4-6(138행) | v2.13.0 `Event.h:80-84` | 200 | 지지 | 80-84행 "Calls record() if and only if record() has never been called for this event. Note: because Event is not thread-safe recordOnce() may call record() multiple times if called from multiple threads." — 인용문과 완전 일치 |
| 8 | Codex A4-16(253행) | main `InlineEvent.h` (recordOnce 구현) | 200 | 지지 | 64-66행 `void recordOnce(...) { if (!was_marked_for_recording_) record(stream); }`, `was_marked_for_recording_`는 평범한 `bool`(락/atomic 아님) — v2.13.0 InlineEvent.h도 동일 코드 확인, "필요시 v2.13 확인" 문구의 우려 해소 |
| 9 | Fable A4-7(139행) | v2.13.0 `VirtualGuardImpl.h`, `DeviceGuardImplInterface.h` | 200(2개) | 지지 | `VirtualGuardImpl.h:8` 주석 "An implementation of DeviceGuardImplInterface which delegates"; `DeviceGuardImplInterface.h:392` `DeviceGuardImplRegistrar` — device-type별 등록·위임 구조 확인 |
| 10 | Fable A4-8(140행), Codex A4-19(259행) | v2.13.0 `Generator.h:133,85`, `GeneratorImpl.h:67,75,102` | 200(2개) | 지지 | Generator.h:133 `Generator clone() const { return Generator(impl_->clone()); }`(무인자), :85 `getIntrusivePtr()`, :104 `set_state(const at::Tensor&)`; GeneratorImpl.h:67 `clone()`(무인자, intrusive_ptr 반환), :75 `set_state(const c10::TensorImpl&)`, :102 `clone_impl()` pure virtual — 인용된 두 계층 혼합 시그니처는 실존하지 않음, 지적대로 두 계층이 섞여 있었음이 확인됨 |
| 11 | Fable A4-9(141행), Codex A4-20(261행) | v2.13.0 `pyproject.toml:15`, `tools/setup_helpers/cmake.py` | 200(2개) | 지지 | pyproject.toml:15 `build-backend = "setuptools.build_meta"`(scikit-build 문자열 전무); setup.py 4-5행 `#   DEBUG` / `#     build with -O0 and -g` — DEBUG는 setup.py가 처리한다는 서술과 일치 |
| 12 | Fable A4-10(142행) | v2.13.0 `cmake/EnvVarForwarding.cmake`, "setup.py:355-361" | 200(2개) | 부분지지 | `EnvVarForwarding.cmake`에 `MAX_JOBS` 별칭 없음(확인됨)은 맞으나, "setup.py:355-361이 직접 읽음"은 파일 착오 — 실제로 355-361행에서 `max_jobs = os.getenv("MAX_JOBS")`를 읽는 코드는 `setup.py`가 아니라 `tools/setup_helpers/cmake.py`에 있음(줄 번호는 정확) |
| 13 | Fable A4-11(143행) | GitHub 이슈 `pytorch/pytorch#152276` | 200 | 부분지지 | 제목은 정확히 일치: "`setup.py develop` command is disappearing soon from `setuptools`". 그러나 본문에는 "current timeline is 'a few months'"만 있고 2.14-2.18 버전별 표나 "spin을 `pip install --group dev`로 설치 예정"이라는 문장은 없음 — 리뷰어 자신이 "needs verification"이라 표시한 것이 정확한 자기평가 |
| 14 | Fable A4-11(143행) 괄호 | v2.13.0 `.spin/cmds.py`, `pyproject.toml` `[tool.spin.commands]` | 200(2개) | 지지 | pyproject.toml:366-381 `[tool.spin.commands]`에 Build(clean/lint/fixlint/quicklint/quickfix)/Regenerate(regenerate_*)/Type(pyrefly) 3그룹만 존재 — "clean/lint/regenerate/pyrefly뿐"이라는 서술과 일치 |
| 15 | Fable A4-17(149행) | v2.13.0 `native_functions.yaml` (mm/add.Tensor), `aten/src/ATen/native/cuda/Blas.cpp:296` | 200(2개) | 지지 | native_functions.yaml 4079-4096행: `mm` 함수에 `SparseCPU, SparseCUDA, SparseMPS: _sparse_mm`(4083행), `mm.out`에 `MTIA: mm_out_mtia`(4092행) 정확히 존재. 536-546행 `add.Tensor`에 `SparseMPS`(541행), `NestedTensorHPU`(544행), `device_check: NoCheck   # TensorIterator`(537행) 모두 확인. `cuda/Blas.cpp:296` `at::cuda::blas::gemm_and_bias<scalar_t, res_scalar_t>(` 정확히 일치(단, 파일 경로가 `aten/src/ATen/native/cuda/Blas.cpp`이지 `native/Blas.cpp`(CPU용)가 아님 — 이 구분은 본문에 명시 안 됐지만 인용 내용 자체는 정확) |
| 16 | Fable A4-15(147행) | v2.13.0 `torch/utils/backend_registration.py`, `c10/core/DispatchKey.h` | 200(2개) | 지지 | `backend_registration.py:76` `def rename_privateuse1_backend(...)`, 83행 주석 "to the PrivateUse1 dispatch key"; `DispatchKey.h:49-51` `PrivateUse1/2/3` 매크로 정의 확인 — out-of-tree backend가 core 패치 없이 예약 key로 등록 가능하다는 지적이 사실과 일치 |
| 17 | Fable A4-18(150행), Codex A4-15(251행) | v2.13.0 `Device.h:181-184`(GitHub blob) | 200 | 지지 | 179행 "Device index must be -1 or non-negative", 182-184행 "CPU device index must be -1 or zero, got..." — "CPU는 -1 또는 0 허용"이라는 두 리뷰어 공통 지적과 정확히 일치(원문 "zero여야 함"은 -1을 빠뜨린 오류) |
| 18 | Fable A4-19(151행) | v2.13.0 `VirtualGuardImpl.h` | 200 | 지지 | `getStreamNativeHandle`(56), `getDeviceCapability`(63), `isStreamCapturing`(92), `elapsedTime`(101), `synchronizeEvent`(106), `synchronizeDevice`(110) 모두 존재 — 표에 빠졌다는 지적이 맞음 |
| 19 | Fable 확인목록(159행) | v2.13.0 `tools/autograd/gen_variable_type.py:940` | 200 | 지지 | 940행 정확히 `num_shards=10,` |
| 20 | Fable 확인목록(159행) | v2.13.0 `torch/csrc/autograd/python_variable.cpp:3627` | 200 | 지지 | 3627행 정확히 `THPVariable_dealloc, /* tp_dealloc */` (3719행이 실제 함수 본문) |
| 21 | Fable 확인목록(159행) | v2.13.0 `README.md` (요구사항) | 200 | 지지 | 165행 "Python 3.10 or later", 166행 "gcc 11.3.0 or newer", 168행 "10 GB", 169행 "30-60 minutes", 248행 `pip install --group dev`, 257행 `install_magma_conda.sh 12.4` — 전부 정확 |
| 22 | Fable 확인목록(159행) | v2.13.0 `native_functions.yaml` `CompositeImplicitAutograd: matmul` | 200 | 지지 | 3726-3730행 `matmul` 함수 `dispatch: CompositeImplicitAutograd: matmul` 확인 |
| 23 | Fable 확인목록(159행) | v2.13.0 `aten/src/ATen/native/cuda/Blas.cpp` `TORCH_IMPL_FUNC(mm_out_cuda)`/`addmm_out_cuda_impl` | 200 | 지지 | 645행 `TORCH_IMPL_FUNC(mm_out_cuda)(...)`, 339행 `addmm_out_cuda_impl(...)` 시그니처 확인(단, `LinearAlgebra.cpp`가 아니라 `cuda/Blas.cpp`에 위치 — 파일명은 본문 확인 안 했으나 내용 자체는 정확) |
| 24 | Codex A4-1(223행) | v2.13.0 `c10/core/DispatchKey.h`(GitHub blob) | 200 | 지지 | 440행 "Alias keys won't be directly called during runtime", 465행 `StartOfAliasKeys = Autograd` — `CompositeImplicitAutograd`·`Autograd` 모두 alias key. `AutogradFunctionality`(63,318행)를 `C10_FORALL_BACKEND_COMPONENTS` 매크로로 전개해 `AutogradCUDA` 등 per-backend runtime key 생성 확인 |
| 25 | Codex A4-5(231행) | v2.13.0 `aten/src/ATen/native/LinearAlgebra.cpp`(raw) | 200 | 지지 | 1921-1975행 `should_fold` 정의, 1986-1994행 docstring: "If one of the arguments is ND with N>=3 and the other is 1D or 2D... we fold... call mm or mv... Otherwise, we return bmm, after broadcasting" — 조건부 fold/mm 설명과 정확히 일치 |
| 26 | Codex A4-6(233행) | v2.13.0 `aten/src/ATen/native/LinearAlgebra.cpp`(raw) | 200 | 지지 | 201-208행 `TORCH_META_FUNC(mm)(...)`가 `TORCH_CHECK(self.dim()==2,...)`와 `set_output_raw_strided`를 직접 작성 — 사람이 쓴 코드이며 codegen은 wrapper만 생성한다는 지적과 일치 |
| 27 | Codex A4-7(235행) | v2.13.0 `aten/src/ATen/cuda/CUDABlas.cpp`(raw) | 200 | 지지 | `gemm_and_bias`(1660행)는 내부적으로 `cublasLtMatmul`(1834행)만 호출, 일반 `gemm_internal_cublas<float>`(1133행)는 `cublasSgemm` 호출 — 두 경로가 다른 함수임을 확인 |
| 28 | Codex A4-8(237행) | v2.13.0 `c10/core/TensorImpl.h`(raw) | 200 | 지지 | 2882-2899행 주석: "autograd_meta_ can be nullptr, as an optimization"이지만 "we don't enforce the invariant that if the AutogradMeta is default constructed, it is nullptr" — requires_grad=False가 항상 null을 뜻하지 않는다는 지적과 정확히 일치 |
| 29 | Codex A4-9(239행) | v2.13.0 `aten/src/ATen/native/TensorFactories.cpp`(raw) | 200 | 지지 | 2235-2256행 `clone()`: `empty_strided_symint`/`empty_like`로 출력 준비 후 `self.copy_(src)` — 지적과 정확히 일치 |
| 30 | Codex A4-11(243행) | v2.13.0 `aten/src/ATen/native/TensorShape.cpp`(raw) | 200 | 지지 | 2009-2033행 `reshape_symint`: contiguous면 view, 아니면 `computeStride`로 view 가능 여부 판단 후 view 또는 (이어지는 코드에서) copy; 4113-4142행 `flatten`은 대부분 `native::reshape_symint` 호출 — "가능하면 공유, 아니면 복사"·"flatten은 내부적으로 reshape 사용" 지적과 일치 |
| 31 | Codex A4-12(245행) | main `docs/source/notes/cuda.md`(raw) | 200 | 지지 | "Memory management" 절: "PyTorch uses a caching memory allocator... the unused memory managed by the allocator will still show as if used in nvidia-smi" — caching allocator가 메모리를 보유한다는 지적과 일치 |
| 32 | Codex A4-13(247행) | v2.13.0 `aten/src/ATen/native/TensorShape.cpp`(raw) | 200 | 지지 | 파일 접근 확인. 2D tensor에 `permute(2,0,1)`(3차원 필요)·`flatten(1,2)`(2번째 축 없음)·`reshape(2,12)`(24개 필요, 6개 보유)는 PyTorch shape 규칙상 명백한 차원 오류 — 별도 3D 예제 필요하다는 지적이 타당 |
| 33 | Codex A4-14(249행) | main `aten/src/ATen/TensorIterator.h`(raw) | 200 | 지지 | 21-23행 "TensorIterator is a helper class for element-wise operations... It handles broadcasting and type conversions of operands." — storage/view 분리 helper가 아니라는 지적과 정확히 일치(Fable A4-13과 동일 결론) |
| 34 | Codex A4-17(255행) | v2.13.0 `c10/xpu/impl/XPUGuardImpl.h`(GitHub blob) | 200 | 지지 | 118-146행 `record(...)`: barrier를 큐에 제출한 반환값으로 `new sycl::event(...)`를 즉시 생성·저장할 뿐, host가 기존 작업 완료를 기다리지 않음 — "host 대기 아님" 지적과 일치 |
| 35 | Codex A4-18(257행) | RBLN 공식 문서 `docs.rbln.ai/v0.11.2/.../api.html` | 200 | 지지 | 페이지 텍스트: "Event Methods: record wait query synchronize", 예제 코드 `torch.rbln.Event()` — v0.11.2에 Event 클래스와 4개 메서드가 명시돼 있어 "아직 event 미지원" 단정이 틀렸다는 지적과 일치 |
| 36 | Codex A3-8(211행) | NVIDIA CUDA Runtime API 문서(`cuda-runtime-api/group__CUDART__EVENT.html`) | 200 | 지지 | 페이지에 `cudaEventRecord`/`cudaEventQuery`/`cudaEventSynchronize`/`cudaEventDestroy`가 서로 다른 API로 나열됨 — "record 완료 ≠ event 소멸"이라는 개념적 근거로 타당(직접 인용 문장은 없어 일반 레퍼런스 성격) |
| 37 | Codex A3-10(215행) | v2.13.0 `caffe2/CMakeLists.txt`(raw) | 200 | 지지 | 1741행 `target_link_libraries(torch_cuda PUBLIC torch_cpu_library ...)`, 1765행 `torch_xpu`도 동일 — 그림에 `torch_cuda → torch_cpu` 의존성이 빠졌다는 지적과 일치 |
| 38 | Codex Axis4 서두(221행) | v2.13.0 `tools/autograd/gen_variable_type.py`(GitHub blob) | 200 | 지지 | #19와 동일 파일, `num_shards=10`(940행) 확인. shard 개수 자체는 맞으나 "add의 정확한 shard 번호·원 캡처 환경은 needs verification"이라는 리뷰어 자신의 유보는 본 검증 범위 밖(코드만으로는 어떤 shard에 `add`가 배정되는지 추가 계산 필요) → 리뷰어의 유보 표시가 적절 |

### 요약

- 총 38건: 지지 34 / 부분지지 2(#12 MAX_JOBS 파일 착오, #13 GitHub 이슈 세부 일정 미확인) / 불일치 0 / 무관 0 / 접근불가 0
- 두 리뷰어의 v2.13.0 태그 소스 인용은 검증된 38건 중 34건이 줄 번호까지 정확히 일치할 정도로 신뢰도가 매우 높음. `DispatchKeyExtractor.h:186-198`, `Event.h:80-84`, `Generator.h:133/85`, `python_variable.cpp:3627`, `gen_variable_type.py:940`, `Dispatcher.h:833/708/777-779` 등은 인용 줄 번호까지 완전히 일치.
- #12(Fable A4-10, MAX_JOBS 읽는 코드 위치를 `setup.py`로 오기 — 실제는 `tools/setup_helpers/cmake.py:355-361`)와 #13(Fable A4-11, GitHub 이슈 #152276의 제목은 정확하나 2.14-2.18 버전별 일정·"spin을 pip install --group dev로 설치" 서술은 이슈 본문에 없음)은 리뷰 결론 자체를 바꿀 정도는 아니며, 특히 #13은 리뷰어가 이미 "needs verification"으로 스스로 표시해 정확히 자기평가한 경우. 수정 반영 시 #12만 파일명을 `tools/setup_helpers/cmake.py`로 교정하면 됨.
- 본 문서에는 논문 인용(Rotenberg, FlashAttention 등)이 존재하지 않음 — 검증 대상에서 자연히 제외.
