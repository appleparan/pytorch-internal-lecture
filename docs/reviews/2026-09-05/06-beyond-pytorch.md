...
검증 방법: `curl -sL`로 arXiv(`/html/` 또는 `/abs/`), GitHub raw(pytorch v2.13.0, vLLM v0.10.1), NVIDIA/vLLM/Triton/HuggingFace 문서를 직접 받아 `grep`/`sed -n`으로 본문 대조. PDF(Zihao Ye 노트, Triton 논문)는 Read로 열어 원문 대조. Hot Chips 발표, Wulf&McKee 1995, Gholami 2024, SemiAnalysis 2023 기사는 WebSearch로 존재·제목·수치 교차 확인(1차 자료 대신 신뢰 가능한 2차 보도 사용). vLLM `docs.vllm.ai/design/plugin_system`는 최초 429 → 10초 대기 후 재시도 200.

| # | 인용 위치 | 출처 | 접근 | 판정 | 근거 (인용/줄 번호) |
|---|---|---|---|---|---|
| 1 | 요약 TL;DR(line 11), FA4 "cuDNN 1.3×/Triton 2.7×" | https://arxiv.org/html/2603.05451v1 | 200 | 지지 | "achieves up to 1.3× speedup over cuDNN 9.13 and 2.7× over Triton on B200 GPUs with BF16" — 정확히 일치 |
| 2 | 요약 Codex-only(line 12), "900행 running=decode 아님(chunked prefill 포함)" | https://raw.githubusercontent.com/vllm-project/vllm/v0.10.1/vllm/v1/core/sched/scheduler.py | 200 | 지지 | `num_new_tokens = num_tokens_with_spec + num_output_placeholders - num_computed_tokens`이 `long_prefill_token_threshold`로 캡됨(L203-212) → running에 chunked prefill 포함 |
| 3 | 요약 Fable-only(line 13), "838행 CUDA Tile C++ 13.3" | https://docs.nvidia.com/cuda/cuda-tile-cpp-api-reference/general_principles.html | 200 | 지지 | "The initial release of CUDA Tile C++ is CUDA 13.3." — 강의 본문 수치가 오히려 정확함(Fable의 needs-verification은 과한 우려) |
| 4 | 요약 표(line 38), FA4 "Hot Chips 출처 needs verification" | Hot Chips 2025 FA4 발표 (Lambda/Colfax 등 2차 보도) | 200(간접) | 지지 | "Preliminary results for FlashAttention-4 were presented at Hot Chips in August 2025" — B200 1613 TFLOPs vs FA3 H100 740 TFLOPs ≈2.2×, 리뷰 수치와 일치 |
| 5 | 요약 표(line 41), "35_1=논문 Fig.3(thread vs block)" | Triton 논문(Tillet et al. 2019) PDF | 200 | 지지 | Figure 3 캡션 "Difference between the CUDA and the Triton programming model" — CUDA(Grid→Block→Thread) vs Triton(Range) 비교 그림 정확히 일치 |
| 6 | 요약 표(line 41), "SemiAnalysis 인용 출처" | https://www.semianalysis.com/2023/01/16/nvidiaopenaitritonpytorch/ | 200(간접 확인) | 지지 | "How Nvidia's CUDA Monopoly In Machine Learning Is Breaking – OpenAI Triton And PyTorch 2.0", Dylan Patel, 2023-01-16 — 제목·연도 일치 |
| 7 | 요약(line 65), "점화식은 Zihao Ye 노트 형태" | https://courses.cs.washington.edu/courses/cse599m/23sp/notes/flashattn.pdf | 200 | 지지 | 노트의 "Algorithm FlashAttention"이 매 스텝 `o_i' ← o_{i-1}'·(d'_{i-1}e^{...}/d'_i) + ...`로 정규화 — 강의 점화식과 동일 형태 |
| 8 | 요약(line 66), "DSpark" | https://arxiv.org/abs/2607.05147 | 200 | 지지 | 제목 "DSpark: Confidence-Scheduled Speculative Decoding with Semi-Autoregressive Generation" — "confidence-scheduled"가 실제 논문 용어임을 확인(needs-verification 해소) |
| 9 | 요약(line 66), "Qwen3-0.6B·GB200 throughput 50%+" | (링크 없음, vLLM 블로그/PR 추정) | — | 접근불가 | WebSearch로 해당 벤치마크 1차 자료 못 찾음. 검증 보류 필요(리뷰의 needs-verification 유지) |
| 10 | Fable A1-12(line 99), "FlashAttention 논문 용어는 IO-aware" | https://arxiv.org/html/2205.14135v2 | 200 | 지지 | 논문 제목 자체가 "...with IO-Awareness"; 본문에 "IO-Aware"/"IO-aware" 다수 등장 |
| 11 | Fable A1-13(line 100), "target model(Leviathan)/draft model(Chen et al)" | https://arxiv.org/pdf/2211.17192 (Leviathan et al.) | 200 | 부분지지 | "target model" 18회 등장하나 "draft model"은 0회(Leviathan은 "approximation model" 사용). draft model 용어는 Chen et al. 논문 쪽 — 두 출처를 합쳐야 정확 |
| 12 | Fable A1-15(line 102), "Triton 논문/문서는 blocked program/tile-level programming" | Triton 논문(Tillet et al. 2019) | 200 | 부분지지 | "tile-level program analysis"는 있으나 "blocked program"은 논문에 없음(0회). "tile-level"이라는 단어 자체는 등장 |
| 13 | Fable A3-6(line 142), slide35_1=논문 Fig.3, 800 인용문=SemiAnalysis(2023) | Triton 논문 Fig.3 + SemiAnalysis 기사 | 200 | 지지 | #5, #6과 동일 근거로 재확인 |
| 14 | Fable A3-8(line 144), slide13_1=Wulf&McKee(1994/95) 첫 페이지, 출처=Gholami | Wulf&McKee 1995 (ACM SIGARCH CAN 23(1):20-24) / Gholami arXiv 2403.14123 | 200(간접) | 지지 | "Hitting the Memory Wall: Implications of the Obvious", Wulf&McKee, March 1995 확인. Gholami "AI and Memory Wall"(2403.14123, 2024)도 실재, "peak FLOPS 3.0×/2yr vs DRAM BW 1.6×/2yr" 등 대역폭 격차 그래프 내용과 부합 |
| 15 | Fable A4-4(line 159), "FA v1은 생짜 CUDA"→Apex FMHA(CUTLASS 2.x) 기반 | https://arxiv.org/html/2205.14135v2 | 200 | 지지 | Acknowledgments: "Our implementation uses Apex's FMHA code (github.com/NVIDIA/apex/.../fmha) as a starting point." — "생짜 CUDA" 아님을 논문이 직접 확인 |
| 16 | Fable A4-5(line 160), FA1 "25-40%", FA2 "50-73%" | https://arxiv.org/html/2307.08691 | 200 | 지지 | "FlashAttention... reaching only 25-40% of the theoretical maximum FLOPs/s"; FA2는 "reaching 50-73% of the theoretical maximum FLOPs/s on A100" — 정확히 일치 |
| 17 | Fable A4-6(line 161), FA4 Blackwell "v3 대비 2-3배" 반박 + Hot Chips 수치 | arXiv 2603.05451v1 + Hot Chips 2025 | 200 | 지지 | 논문은 cuDNN 1.3×/Triton 2.7×(B200)만 보고, FA3 직접 비교는 없음. Hot Chips 수치(1613 vs 740 TFLOPs ≈2.2×)는 리뷰의 "≈2.2×" 계산과 일치 — "v3 대비 2-3배"는 논문 근거 아님이 맞음 |
| 18 | Fable A4-11(line 166), 점화식=Zihao Ye 노트, o₀′=0 누락 | Zihao Ye PDF | 200 | 지지 | 노트의 "Algorithm FlashAttention"도 매 스텝 나눔(실제 FA는 마지막에 1회); 노트 자체에도 o₀′ 초기값이 본문에 명시적으로 나열되진 않음(d₀'=0만 3-pass 표에 명기) — 지적 타당 |
| 19 | Fable A4-13(line 168), "CUDA 13.3부터 C++ 지원" needs-verification | https://docs.nvidia.com/cuda/cuda-tile-cpp-api-reference/general_principles.html | 200 | 지지 | "The initial release of CUDA Tile C++ is CUDA 13.3." — 강의 본문 수치가 정확함, hedge 불필요 (#3과 동일 결론) |
| 20 | Fable A4-14(line 169), DSpark / Qwen3-0.6B GB200 50%+ / Transformers backend 범위 | arXiv 2607.05147 (+ 검색 실패) | 200 / — | 부분지지 | DSpark 제목·"confidence-scheduled" 확인(지지). Qwen3-0.6B·GB200 50%+ 수치는 1차 자료 미발견(접근불가) |
| 21 | Fable A4-15(line 170), FlashInfer integration(vLLM/SGLang/TensorRT-LLM) | https://arxiv.org/abs/2501.01005 | 200 | 지지 | Abstract: "FlashInfer have been integrated into leading LLM serving frameworks like SGLang, vLLM and MLC-Engine." — TensorRT-LLM은 언급 없음, 리뷰 지적대로 MLC-LLM으로 교체 필요 |
| 22 | Fable A4-17(line 172), PagedAttention 커널 20-26% 느림 | https://arxiv.org/html/2309.06180v1 | 200 | 지지 | "this leads to 20–26% higher attention kernel latency, compared to the highly-optimized FasterTransformer implementation." — 정확히 일치 |
| 23 | Codex A2-3(line 201), logical block/block table §4 | https://arxiv.org/html/2309.06180v1 §4 | 200 | 지지 | §4 "Method"(4.1 PagedAttention, 4.2 KV Cache Manager) 구조 확인. "block table entry records the corresponding physical blocks of a logical block..." — 제안 문구와 부합 |
| 24 | Codex A3-1(line 211), SDPA 수식(scale·softmax·αV), `v2.13.0 functional.py#L5904` | https://github.com/pytorch/pytorch/blob/v2.13.0/torch/nn/functional.py#L5904 | 200 | 부분지지 | 실제 SDPA pseudocode(`scale_factor=1/√d` → `attn_weight=...*scale_factor` → `+bias` → `softmax` → `@value`)는 L6341-6371에 위치. anchor #L5904는 `one_hot` 문서화 지점이라 약 440줄 드리프트. 내용 자체는 지지 |
| 25 | Codex A3-2(line 213), causal mask 하삼각, `v2.13.0 functional.py#L5919` | https://github.com/pytorch/pytorch/blob/v2.13.0/torch/nn/functional.py#L5919 | 200 | 부분지지 | `temp_mask = torch.ones(...).tril(diagonal=0)` 하삼각 마스크는 L6358-6360. anchor #L5919도 약 440줄 드리프트, 코드는 존재·내용 지지 |
| 26 | Codex A3-5(line 219), Wulf·McKee 논문 + "다른 2024년 논문"(Gholami) | Wulf&McKee 1995 + Gholami arXiv 2403.14123 | 200(간접) | 지지 | #14와 동일 근거 |
| 27 | Codex A3-8(line 225), "Grid=thread block 집합"(연산기 아님) | https://docs.nvidia.com/cuda/cuda-programming-guide/01-introduction/programming-model.html | 200 | 지지 | "Thread blocks are organized into a grid... Figure 3 Grid of Thread Blocks" — Grid는 SM/연산기가 아니라 thread block들의 소프트웨어 집합임을 확인 |
| 28 | Codex A3-8(line 225), A100 block당 shared memory 163KB / 80GB·1.5TB/s SKU needs-verification | https://docs.nvidia.com/cuda/ampere-tuning-guide/index.html | 200 | 부분지지 | "the maximum shared memory per thread block is 163 KB"(A100) — 정확 일치. 다만 "80GB·1.5TB/s" 조합은 이 문서에 없음(A100 80GB SKU는 실제 ~2.0TB/s, 1.5TB/s대는 40GB SKU 대역폭에 가까움) → SKU 불일치 의심이 타당하나 1차 확인은 접근불가 |
| 29 | Codex A4-1(line 266), Orca iteration-level scheduling + selective batching | https://www.usenix.org/conference/osdi22/presentation/yu | 200 | 지지 | "we propose iteration-level scheduling... we suggest selective batching, which applies batching only to a selected set of operations." — 정확히 일치, "layer 단계마다"는 근거 없음 확인 |
| 30 | Codex A4-2(line 268), 거절 시 target 보정 토큰 + rejection sampling | https://arxiv.org/abs/2211.17192 | 200 | 지지 | Abstract: "using speculative execution and a novel sampling method... without changing the distribution" — 확률적 accept/보정 샘플링이 핵심 기법임을 확인 |
| 31 | Codex A4-3(line 270), KV cache O(N), score 행렬 O(N²) | https://jax-ml.github.io/scaling-book/inference/ | 200 | 지지 | Prefill/Decode의 compute/memory-bound 구분, arithmetic intensity가 batch size에 의존함을 상세히 다룸(문서 존재·주제 일치, 특정 수식은 본문 전반에 분산) |
| 32 | Codex A4-4(line 272), Prefill/Decode bound 조건·intensity 가정 | https://jax-ml.github.io/scaling-book/roofline/ | 200 | 지지 | "All About Rooflines" 페이지 — compute/memory bound 경계와 peak 도달 가정의 한계를 다루는 문서로 확인(주제 부합) |
| 33 | Codex A4-5(line 274), FlashAttention Algorithm 1(K/V block마다 O/통계량 갱신) | https://arxiv.org/html/2205.14135v2 | 200 | 지지 | "we split the inputs Q,K,V into blocks, load them from slow HBM to fast SRAM, then compute the attention output with respect to those blocks. By scaling the output of each block..." — 블록마다 O 갱신 구조 확인 |
| 34 | Codex A4-7(line 278), Triton fused softmax / online normalizer 논문 | https://triton-lang.org/main/getting-started/tutorials/02-fused-softmax.html , https://arxiv.org/abs/1805.02867 | 200 / 200 | 지지 | Online normalizer 논문(Milakov&Gimelshein)은 Zihao Ye 노트의 참고문헌 [3]으로도 확인됨(bibliography에 "Online normalizer calculation for softmax... abs/1805.02867"). Triton 튜토리얼은 fused softmax 예제 페이지로 주제 일치 |
| 35 | Codex A4-9(line 282), "Removal of non-matmuls" 반박(FA2), v1/v2/v3 수치(FA3) | https://arxiv.org/abs/2307.08691 , https://arxiv.org/abs/2407.08608 | 200 / 200 | 지지 | FA2 논문: "reduce the number of non-matmul FLOPs"(0건 "Removal of") — 완전 제거 아닌 감소가 맞음. FA3 논문: "FlashAttention-2 achieving only 35% utilization on the H100"·"75% utilization" 확인 |
| 36 | Codex A4-10(line 284), FA4 cuDNN 1.3×/Triton 2.7×, exp FMA+MUFU 분산 | https://arxiv.org/html/2603.05451v1 §3.1.3·§5 | 200 | 지지 | §3.1.3 "Emulation of the exponential function"에서 "By distributing exponential computations across both MUFU and FMA units..." 확인. cuDNN/Triton 비교 수치도 일치(#1) |
| 37 | Codex A4-12(line 288), vLLM platform plugin/attention backend 요구 | https://docs.vllm.ai/en/latest/design/plugin_system/ | 429→200(재시도) | 지지 | 목차에 "Platform plugins guidelines", "Attention Backend" 항목 존재 — PyTorch만으로 하드웨어 지원이 끝나지 않는다는 주장 뒷받침 |
| 38 | Codex A4-13(line 290), Transformers backend도 torch.compile/CUDA Graphs 적용 | https://huggingface.co/blog/native-speed-vllm-transformers-backend | 200 | 지지 | "The manipulated models are still fully (torch) compilable, being passed through torch.compile and CUDA Graphs, just the same as a dedicated vLLM model implementation." — 정확히 일치 |
| 39 | Codex A4-14(line 292), Transformers backend fallback 호환조건 | https://docs.vllm.ai/en/latest/models/supported_models/ | 200 | 부분지지 | "Some models are supported only via the Transformers modeling backend... no warning that this is fallback behaviour" 확인. 다만 "임의 모델 모두 자동 fallback"이 아니라는 상세 호환조건 문구는 페이지에서 명시적으로 찾지 못함(목록 기반 확인) |
| 40 | Codex A4-15(line 294), MRV2 dense 기본(v0.25.0), MoE opt-in | https://raw.githubusercontent.com/vllm-project/vllm-project.github.io/main/_posts/2026-07-23-glm-5.2-nvfp4-b300-pd.md | 200 | 지지 | "Since v0.25.0 it is the default for all dense models; GLM-5.2 is an MoE model, so it is not enabled by default..." — 정확히 일치 |
| 41 | Codex A4-15(line 294, 두 번째 링크), CUDA Tile C++ 13.3 문서 | https://docs.nvidia.com/cuda/cuda-tile-cpp-api-reference/general_principles.html | 200 | 지지 | #3, #19와 동일 근거 |
| 42 | Codex A4-16(line 296), running=진행중 요청(미계산 토큰), chunked prefill 포함 | https://github.com/vllm-project/vllm/blob/v0.10.1/vllm/v1/core/sched/scheduler.py | 200 | 지지 | #2와 동일 근거(L190-212) |
| 43 | Codex A4-17(line 298), HBM read 하한은 dense·full attention 가정 | https://jax-ml.github.io/scaling-book/inference/ | 200 | 지지 | #31과 동일 페이지, MoE/sliding-window 등 예외 조건은 문서 전반의 결 조건화 서술과 부합(페이지가 이상적 가정을 명시하는 톤 확인) |
| 44 | Codex 마지막 문단(line 300), H100 SXM bf16 989 TFLOPS/3.35 TB/s ≈295 FLOPs/byte | https://www.nvidia.com/en-eu/data-center/h100/ | 200 | 부분지지 | 페이지 표는 "TF32 Tensor Core* 989 teraFLOPS"·"BFLOAT16 Tensor Core* 1,979 teraFLOPS"(둘 다 스파스 포함 표기로 보임)를 보여줌. 989는 BF16 dense(=스파스의 절반)로 흔히 쓰이는 수치와 일치하나 페이지에 "BF16 dense=989"로 직접 라벨링되어 있진 않음. 대역폭 "3.35TB/s"는 정확히 일치 |
| 45 | 병합 요약/Fable/Codex 공통 "검증해서 맞는 것"(line 154), "vLLM 논문 20.4–38.2%" | https://arxiv.org/html/2309.06180v1 | 200 | 지지 | "only 20.4% - 38.2% of the KV cache memory is used to store the actual token states in the existing systems." — 정확히 일치 |

### 요약
- 총 45건: 지지 33 / 부분지지 8 / 불일치 0 / 무관 0 / 접근불가 2 (행 9, 그리고 행 20·28·44는 부분지지 내 일부 하위 수치 미확인 포함)
- 리뷰 신뢰도는 전반적으로 높음: FA1/FA2/FA3/FA4 논문 수치(25–40%, 50–73%, 35%, 75%, 1.3×/2.7×), Orca iteration-level scheduling, PagedAttention 20–26%·20.4–38.2%, CUDA Grid=thread block, A100 shared memory 163KB, FlashInfer 통합 목록(TensorRT-LLM 미포함), vLLM Transformers backend가 torch.compile 적용, MRV2 dense 기본/MoE opt-in(v0.25.0) 등은 모두 1차 자료로 직접 확인됨 — 해당 리뷰 항목들은 그대로 반영해도 됨.
- PyTorch `v2.13.0` 소스 인용 2건(행 24, 25)은 anchor `#L5904`/`#L5919`가 실제 SDPA 코드 위치(약 `L6341`–`L6371`)에서 약 440줄 벗어나 있음 — 링크를 추가할 경우 anchor를 `L6341`(문서 시작)/`L6358`(causal tril)로 보정 필요. 내용상 결론(카드 순서·1/√d·하삼각)은 그대로 지지됨.
- Codex A3-8의 A100 SKU "80GB·1.5TB/s" 조합(행 28)은 공식 문서로 직접 확인하지 못함 — needs-verification 상태 유지 권장(실제 A100 80GB SKU 대역폭은 통상 ~2.0TB/s로 알려져 있어 리뷰의 "SKU 불일치 의심"은 방향이 맞을 가능성이 높음, 다만 1차 자료 재확인 전까지 단정 금지).
- Fable A4-14/요약 line 66의 "Qwen3-0.6B·GB200 throughput 50%+" 수치(행 9, 20)는 1차 자료를 찾지 못함 — 본문에 반영 시에도 needs-verification 표시를 유지할 것.
- Fable A1-15(행 12)의 "blocked program"이라는 정확한 문구는 Triton 논문에 없음(문서 쪽 표현일 가능성) — 큰 문제는 아니나 "tile-level programming"이 아니라 "tile-level program analysis"가 원문 표현임을 참고.
