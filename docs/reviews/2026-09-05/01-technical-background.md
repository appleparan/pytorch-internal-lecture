...
검증 방법: PyTorch 소스는 `curl -sL raw.githubusercontent.com/pytorch/pytorch/v2.13.0/...` (pin된 태그), PDF는 `curl` + `pdftotext`, 나머지 docs/blog 페이지는 WebFetch(1차) 실패 시 `curl`로 재시도. `docs.pytorch.org/docs/stable/*`는 클라이언트 리다이렉트(`location.replace`)로 실제 `2.14` 버전을 가리켜 이 문서가 리뷰 대상으로 삼는 PyTorch 2.13과 다름을 확인함(각 행에 표시). jax.dev는 Cloudflare 429로 10초 대기 후 재시도까지 실패해 접근불가 처리.

| # | 인용 위치 | 출처 | 접근 | 판정 | 근거 (인용/줄 번호) |
|---|---|---|---|---|---|
| 1 | Codex A4-1 (line 172) | https://docs.pytorch.org/docs/stable/user_guide/torch_compiler/torch.compiler.html | 200 (단, `stable`→`2.14`로 리다이렉트, 2.13 아님) | 지지 | "TorchDynamo ... uses a CPython feature called the Frame Evaluation API to safely capture PyTorch graphs. ... TorchInductor is the default torch.compile deep learning compiler that generates fast code" — Dynamo=graph capture frontend, Inductor=코드 생성 backend라는 정정 주장을 그대로 뒷받침 |
| 2 | Codex A4-2 (line 60) | https://www.tensorflow.org/guide/intro_to_graphs | 200 | 부분지지 | 페이지는 "define-by-run"/"define-and-run" 용어 자체를 쓰지 않음. 다만 "TensorFlow operations are executed by Python, operation by operation"(eager) vs "Graph execution means that tensor computations are executed as a TensorFlow graph"로 개념적 순서(즉시 실행 vs 사전 구성 후 실행)는 뒷받침. 용어 매핑 자체를 확인하려면 다른 출처 필요 |
| 3 | Codex A4-3 (line 52) | https://www.mpi-forum.org/docs/mpi-1.1/mpi1-report.pdf | 200 | 지지 | "Version 1.0: June, 1994. The Message Passing Interface Forum (MPIF)... has been meeting since January 1993" (본문 22-23행). 부록에는 1992년 4월 워크숍·11월 워킹그룹 회의 기록도 있어 "1992=Forum 결성/논의 시작, 1994=MPI-1.0 표준"을 정확히 뒷받침 |
| 4 | Codex A4-4 (line 54) | https://tvm.apache.org/history | 200 | 지지 | "TVM began as a research project at the SAMPL group of Paul G. Allen School of Computer Science & Engineering, University of Washington." / "The project is now an effort undergoing incubation at The Apache Software Foundation (ASF)" — UW 시작·Apache 프로젝트라는 정정 그대로 확인 |
| 5 | Codex A4-5 (line 92) | https://research.nvidia.com/publication/2005-08_survey-general-purpose-computation-graphics-hardware | 200 | 지지 | 게시일 "Monday, August 1, 2005" — CUDA 등장(2007)보다 GPGPU 연구가 먼저 있었다는 주장을 뒷받침 |
| 6 | Codex A4-6 (line 131-134) | https://www.netlib.org/blas/blas.pdf | 200 (curl로 재수집; WebFetch는 PDF 바이너리 실패) | 지지 | "Level 1 BLAS: vector, O(n) operations" / "Level 3 BLAS: matrix-matrix, O(n3) operations" (2, 386행) — 정방행렬일 때만 O(N³)이고 일반적으로 Θ(MKN)이라는 지적과 BLAS 레벨 분류를 뒷받침 |
| 7 | Codex A4-7 (line 138-139) | https://developer.nvidia.com/blog/cutlass-linear-algebra-cuda/ | 200 (curl로 재수집; WebFetch 1차 빈 응답) | 지지 | JSON-LD 메타데이터 `"datePublished":"2017-12-06T04:03:29+00:00"` — 2018이 아니라 2017년 공개. 본문 "We are releasing our CUTLASS source code on GitHub as an initial exposition of CUDA GEMM techniques that will evolve into a template library" — BLAS API 구현이 아니라 GEMM용 CUDA C++ 템플릿 모음이라는 정정도 뒷받침 |
| 8 | Codex A4-8 (line 144-149) | https://docs.pytorch.org/docs/stable/user_guide/torch_compiler/torch.compiler_dynamic_shapes.html | 200 (`stable`→`2.14`) | 지지 | "Dynamic shapes allow PyTorch models to handle inputs with varying dimensions without recompilation... Dynamic shapes work by symbolically tracing tensor dimensions" — "정적 model로 고정"이 아니라는 정정을 직접 뒷받침 |
| 9 | Codex A4-8 (line 144-149) | https://docs.pytorch.org/docs/stable/generated/torch.compile | 200 (`stable`→`2.14`) | 지지 | "for every frame executed within the compiled region, we will attempt to compile it... Multiple compiled results can be associated with a frame up to torch._dynamo.config.recompile_limit... at which point we will fall back to eager" — graph break로 eager와 섞일 수 있다는 정정을 뒷받침 |
| 10 | Codex A4-9 (line 212) | https://numpy.org/doc/stable/user/c-info.html | 200 | 부분지지 | 페이지 제목 "Using NumPy C-API"이며 본문에 `pybind` 문자열이 전혀 없음(색인 성격 페이지라 상세 서술은 하위 페이지에 있음). NumPy가 pybind11이 아닌 자체 C-API를 쓴다는 방향은 일치하지만, 이 페이지 자체의 산문 인용은 부족 |
| 11 | Codex A4-9 (line 212) | https://github.com/pytorch/pytorch/blob/v2.13.0/torch/csrc/autograd/python_variable.cpp (raw로 검증) | 200 (v2.13.0 태그 실재) | 지지 | 10행 `#include <pybind11/pytypes.h>`, 3354행 `pybind11::gil_scoped_release no_gil;` — pybind11 사용 확인. 동시에 408행 `Py_TYPE(obj)->tp_name`, 3613-3664행 `tp_init`/`tp_new` 등 CPython C API도 직접 사용 — "PyTorch는 pybind11 외에 직접 C API도 사용"까지 정확히 뒷받침 |
| 12 | Codex A4-10 (line 228) | https://docs.pytorch.org/tutorials/intermediate/torch_compile_full_example.html | 200 | 지지 | "torch.compile makes PyTorch code run faster by JIT-compiling PyTorch code into optimized kernels" — `torch.compile`이 AOT가 아니라 JIT라는 정정을 공식 문서 문구로 직접 뒷받침 |
| 13 | Codex A4-10 (line 228) | https://docs.pytorch.org/docs/stable/user_guide/torch_compiler/torch.compiler.html (1번과 동일 URL) | 200 (`stable`→`2.14`) | 지지 | 1번과 동일 근거(Dynamo/Inductor 역할 설명)가 "ahead-of-time" 오표현 정정에도 그대로 적용됨 |
| 14 | Codex A4-11 (line 235-239) | https://www.tensorflow.org/guide/function | 200 | 지지 | "In the first stage, referred to as 'tracing'... all TensorFlow operations... are captured... and not run." / "if you repeatedly call a tf.function with the same argument type, TensorFlow will skip the tracing stage and reuse a previously traced graph" — tracing 후 graph 재사용이라는, PyTorch eager autograd(매번 기록)와 다른 방식이라는 주장을 뒷받침 |
| 15 | Codex A4-11 (line 235-239) | https://docs.jax.dev/en/latest/jit-compilation.html | 접근불가(429) | 접근불가 | Cloudflare가 두 차례(최초+10초 후 재시도) 모두 429/챌린지 페이지 반환. jax.jit 관련 절반 주장은 TF 쪽 근거(#14)로만 간접 확인, JAX 고유 서술은 미검증 |
| 16 | Codex A4-12 (line 313-320) | https://shiftleft.com/mirrors/www.hpl.hp.com/techreports/93/HPL-93-35.pdf | 200 | 지지 | "Trace scheduling is an optimization technique that selects a sequence of basic blocks as a trace and schedules the operations from the trace together. If an operation is moved across basic block boundaries, one or more compensation copies may be required" (18-20행) — trace scheduling 정의와 "보상 코드" 개념을 정확히 뒷받침. 다만 이 논문은 Fisher 1981 원논문이 아니라 1993년 HP 후속 연구(Freudenberger·Gross·Lowney)임 |
| 17 | Codex A4-12 (line 313-320) | https://v8.dev/blog/launching-ignition-and-turbofan | 200 | 지지 | "The new pipeline is built upon Ignition, V8's interpreter, and TurboFan, V8's newest optimizing compiler" / "simplifying the deoptimization mechanism that is a key feature of V8's adaptive optimization" — V8은 인터프리터+최적화 컴파일러+deopt 구조이지 trace 기반 JIT가 아니라는 Fable A4-7/Codex 지적을 뒷받침 |
| 18 | Codex A4-13 (line 328) | https://shiftleft.com/mirrors/www.hpl.hp.com/techreports/93/HPL-93-35.pdf (16번과 동일) | 200 | 지지 | 같은 인용("compensation copies may be required")이 "롤백 없이"가 아니라 "보상 코드"가 정확한 개념이라는 주장을 뒷받침 |
| 19 | Codex A4-14 (line 353) | https://pytorch.org/blog/optimizing-cuda-rnn-with-torchscript/ | 200 | 지지 | "we officially released PyTorch 1.1... better support for fast, custom Recurrent Neural Networks (fastrnns) with TorchScript" + forward/backward 각각의 최적화 전후 시간(예: backward 64ms→27ms)을 상세히 비교 — 2.0 이전에도 TorchScript가 training(forward+backward)을 지원했다는 정정을 직접 뒷받침 |
| 20 | Codex A4-15 (line 359-363) | https://docs.pytorch.org/docs/stable/user_guide/torch_compiler/export/api_reference.html | 200 (title: "torch.export API Reference — PyTorch 2.14 documentation") | 지지 | 페이지 존재 확인, 버전은 2.14(2.13 아님) — 결론 표의 통합 지점을 접점별로 나누자는 제안의 근거 페이지로 유효하나 2.13 문서로 재확인 필요하다는 리뷰어 자신의 단서와 일치 |
| 21 | Codex A4-15 (line 359-363) | https://docs.pytorch.org/tutorials/recipes/torch_export_aoti_python.html?highlight=backward | 200 | 지지 | 페이지 자체가 "PyTorch v2.14.0+cu130" 버전 표시. "With torch.compile() being a JIT compiler, there is a warmup cost... With AOTInductor, the compilation is done ahead of time" — torch.compile=JIT, AOTInductor=AOT라는 구분과, 리뷰어가 스스로 "이 문서는 2.14"라고 단 단서 둘 다 정확 |
| 22 | Codex A4-16 (line 379-381) | https://docs.nvidia.com/cuda/cuda-programming-guide/03-advanced/advanced-kernel-programming.html?hl=en-US | 200 | 부분지지 | "Parallel Thread Execution (PTX), the virtual machine instruction set architecture (ISA) that CUDA uses to abstract hardware ISAs" — PTX가 프로그래밍 모델이 아니라 가상 ISA라는 핵심 주장은 뒷받침. 다만 thread/block/grid 모델 자체는 이 페이지가 아니라 별도 섹션("Writing SIMT Kernels")을 참조하라고 되어 있어 이 URL 단독으로는 완전한 근거가 아님 |
| 23 | Codex A4-16 (line 379-381) | https://triton-lang.org/main/programming-guide/chapter-1/introduction.html | 200 | 부분지지 | "we specifically revisit... 'Single Program, Multiple Data' (SPMD) execution models for GPUs, and propose a variant in which programs – rather than threads – are blocked" — block 단위 프로그래밍 모델 주장은 뒷받침. 그러나 "Triton도 결국 PTX로 컴파일된다"는 부분은 이 페이지에서 언급되지 않음(코드젠/백엔드 문서 별도 확인 필요) |
| 24 | Codex A4-17 (line 372) | https://pytorch.org/blog/pytorch-2-12-release-blog/ | 200 | 지지 | "Torchscript was deprecated in 2.10 and torch.export should be used to replace the jit trace and script APIs" — TorchScript deprecated, torch.export 권장이라는 서술을 뒷받침(단, 최초 deprecated 시점은 2.10이며 이 2.12 포스트는 그 사실을 재확인하는 문서) |
| 25 | Axis4 일반 메모 | https://docs.vllm.ai/en/stable/design/plugin_system/ | 200 | 지지 | "C++ ops. This kind of ops are implemented in C++ and are registered as torch custom ops." / "Most ops can be run by pytorch native implementation... you can implement specific custom ops for your plugins" — vLLM 플러그인이 PyTorch custom operator를 활용한다는 서술과 부합. 다만 이 페이지는 PyTorch를 "narrow waist" 하드웨어 추상화로 명시적으로 프레이밍하지는 않음(그 표현은 별도의 vLLM 트윗/이미지에서 온 것) |

### 요약
- 총 25건: 지지 20 / 부분지지 4 / 불일치 0 / 무관 0 / 접근불가 1
- #2(TF eager/graph 문서)는 "define-by-run/define-and-run" 정확한 용어를 담고 있지 않음 — Codex A4-2(line 60 순서 반전 지적)는 개념적으로는 맞지만, 이 용어 자체의 출처로는 Chainer/PyTorch 공식 문서 등 더 직접적인 출처를 다시 인용해야 함.
- #10(NumPy C-API 색인 페이지)은 상세 산문이 없는 목차 페이지 — A4-9의 "NumPy는 pybind11 아님" 결론 자체는 GitHub 소스 검증(#11)으로 충분히 지지되므로 유지하되, NumPy 쪽 인용은 `c-info.how-to-extend.html` 같은 하위 페이지로 교체 권장.
- #15(JAX 공식 문서, 429 접근불가)로 인해 A4-11의 "JAX도 tracing 후 재사용" 절반 주장은 미검증 상태 — TF 쪽(#14)만 확인됐으므로 재검토 시 재시도하거나 JAX 논문/블로그로 대체 필요.
- #22, #23(CUDA/Triton 공식 문서)은 각각 "PTX=가상 ISA"와 "Triton=block 단위 SPMD"라는 핵심 주장만 뒷받침하고, "thread/block/grid 모델"·"Triton도 PTX로 컴파일"이라는 나머지 절반은 이 URL들에 없음 — A4-16 제안문을 채택하려면 CUDA의 SIMT 섹션과 Triton 백엔드/코드젠 문서를 추가로 인용해야 함.
- #16/#18(HPL-93-35.pdf)은 Fisher 1981 원논문이 아니라 1993년 후속 연구임 — "trace scheduling=compensation code" 개념 자체는 정확히 뒷받침하지만, 리뷰 문서에서 "Fisher 논문"이라 부른 이미지(slide16_1.png, 1981)와 이 인용 논문(1993)을 혼동하지 않도록 구분 필요.
