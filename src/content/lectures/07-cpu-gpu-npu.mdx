...
> Systolic Array는 VLIW와는 꽤 다른 형태의 가속기로 보는 편이 정확합니다. 더 극단적으로 프로그래머빌리티 측면의 제약이 크지만, 하드웨어 효율 자체는 훨씬 높습니다. 가지고 있는 프로그래밍 모델 자체가 VLIW와 다르기 때문에, "VLIW가 유리하냐 아니냐"의 문제라기보다는 별도의 갈래로 이해하는 것이 맞습니다.
...
둘째, 5세대 Tensor Core 명령어 **`tcgen05.mma`** 는 발행 방식 자체가 바뀌었습니다. Hopper의 `wgmma`는 그래도 warp-group의 128개 스레드가 함께 참여하는 형태였지만, `tcgen05.mma`는 **선출된 스레드 하나가 명령을 발행하면 하드웨어가 tile 전체의 MMA를 알아서 수행**하고, 완료되면 mbarrier로 신호를 보내 주는 완전 비동기 동작입니다. SMEM에 있는 입력 tile들의 위치와 형상은 descriptor로 기술해서 넘깁니다. 이 시점에서 Tensor Core는 더 이상 "스레드들이 나눠 밟는 SIMD 연산 유닛"이 아니라, TMA와 대칭을 이루는 **독립된 연산 엔진**입니다. 로드 엔진(TMA)에 "이 영역 옮겨 놔"라고 명령을 발행하듯, 연산 엔진(Tensor Core)에 "이 tile 곱해 놔"라고 명령을 발행하고 완료 신호를 기다리는 구조가 된 것입니다. 여기에 `cta_group::2` 모드는 이웃한 두 CTA를 짝지어 하나의 더 큰 output tile을 협력 생산하게 하는, SM 경계를 넘는 정적 협력까지 추가합니다.
