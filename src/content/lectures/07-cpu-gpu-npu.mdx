...
Blackwell은 이 방향을 하드웨어 구조로 한 걸음 더 밀어붙입니다. 누적(accumulator) 값을 Register File에서 아예 빼내 TMEM이라는 전용 온칩 메모리로 옮겼고(레지스터 압박 완화, 즉 Occupancy 예산 문제의 하드웨어적 인정), 5세대 Tensor Core(`tcgen05`)는 단일 스레드가 tile 단위 MMA를 통째로 발행하는 형태가 되었습니다. "수천 개 스레드가 각자 조금씩"이라는 SIMT의 원형에서, "소수의 역할 분담된 유닛에 큰 작업을 명시적으로 발행"하는 NPU식 그림으로 ISA 수준에서 이동하고 있는 것입니다.
