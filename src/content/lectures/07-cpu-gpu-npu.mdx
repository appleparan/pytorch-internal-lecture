...
- 수렴 - Hopper/Blackwell 세대의 SOTA 커널이 보여주는, GPU 위에서 다시 등장하는 SW 주도 Latency Hiding
...
이 스택에서 Programming Language와 Compiler는 소프트웨어 영역에 해당하고, ISA(Instruction Set Architecture)와 Control Logic, Datapath는 하드웨어 영역에 해당합니다. 특히 Control Logic과 Datapath를 합쳐서 **Microarchitecture**라고 부릅니다.
...
3. **최적화 목적에 맞는 Programming Model 제공** *(그림의 ③: Developer까지 노출)*: Datapath의 중요한 특성을 개발자에게 직접 노출하고, 그 디테일을 잘 다룰 수 있는 프로그래밍 모델을 함께 제공하여 개발자가 하드웨어 특성을 고려하며 코드를 작성하도록 지원합니다.
...
이 간극, 즉 Latency와 Throughput이 항상 직접적으로 연결되어 있지는 않다는 사실을 활용하는 것이 바로 **Latency Hiding**입니다. 이제 이 Latency Hiding을 통해 Throughput을 끌어올리는 여러 마이크로아키텍처 기법들(Instruction-level / Loop-level / Thread-level pipelining)을 차례로 살펴봅니다.
...
Branch는 생각보다 굉장히 빈번하게 일어납니다. 워드프로세서·웹브라우저처럼 일반적으로 "Integer program"이라 부르는 워크로드에서는 6~7개 명령어 중 하나는 Branch일 정도입니다. 이때마다 Branch Latency가 그대로 노출되어 버블이 생긴다면 성능에 큰 영향을 줍니다. 그래서 Delay Slot처럼 ISA의 의미를 바꾸는 트릭이 아니라, 하드웨어 자체에 별도 장치를 두어 Branch Latency를 숨기는 방식이 등장합니다. 그 가장 보편적이고 유명한 장치가 바로 **Branch Prediction**입니다.
...
왼쪽은 루프 자체(`loop:` 레이블이 붙어 반복 시작점이 되는 코드)이고, 오른쪽은 그 루프를 실제로 풀어서(unroll) 순차 실행했을 때의 모습입니다. 마지막 줄에 있는 BLT(Branch if Less Than)가 조건부 분기 명령어로, 루프의 끝에서 다시 처음으로 되돌아가는 **Back Edge**를 형성합니다. 문제는 이 조건부 분기가 해결되어야만 다음 iteration을 시작할 수 있다는 점입니다.
...
3. **Register Renaming → Data-flow graph 전환** - 펼쳐진 trace에는 같은 레지스터 이름이 iteration마다 반복 등장하지만, 하드웨어가 이름을 바꿔 주면 False Dependency가 사라집니다. 그 결과 명령어들 사이에는 오직 True Dependency만 남고, 전체 코드는 의존 관계를 표현하는 **data-flow graph(DAG)** 형태로 정리됩니다. 이 DAG에서는 topological order로 묶을 수 있는 명령어들은 동시에 실행해도 결과가 같기 때문에, 자연스럽게 병렬 처리가 가능해집니다.
...
OOO Processor는 명령어가 들어올 때마다 코드에 적힌 Architectural Register 이름을, 그 시점에 비어 있는 Physical Register 중 하나에 매핑합니다. 그리고 그 명령어가 같은 Architectural 이름을 읽을 때는, 매핑 테이블을 따라가서 올바른 값이 들어 있는 Physical Register를 가리키도록 만듭니다. 이름은 그대로 `t1`이지만, 실제로는 iteration 1의 `t1`과 iteration 2의 `t1`이 서로 다른 물리 저장소에 들어가 있는 셈입니다.
...
이 절의 제목인 "Very Long Instruction Word"는 이름 그대로, 한 cycle에 하드웨어 전체에게 무엇을 시킬지가 한 줄에 길게 적혀 있는 명령어 형식을 가리킵니다. 이 형식을 인코딩 관점에서 보면, 앞서 Instruction-Level Pipelining 절에서 다뤘던 Vertical Encoding의 정반대 방향에 위치합니다. 그래서 이 방식을 흔히 **Horizontal Encoding**이라고 부릅니다.
...
최근에는 VLIW를 **Reconfigurable Processor**라고 부르기도 합니다. 이름이 바뀌는 이유는 그렇게 거창하지 않습니다. Horizontal Encoding의 본질을 다시 떠올려 보면, 명령어가 하드웨어의 각 부분이 매 cycle 무엇을 할지를 직접 지정하고 있죠. 이건 보기에 따라 그 cycle에 하드웨어를 어떤 모양으로 작동하게 할지를 소프트웨어가 매번 다시 그려 주는 것과 같습니다. 즉 매 cycle마다 하드웨어를 다른 용도로 재구성(reconfigure)하는 셈인 것입니다.
...
- **Out-of-Order Processor**에서는 하드웨어가 동적으로 자연스럽게 이 효과를 만들어냅니다. 그 이론적 기반은 1960년대의 **Tomasulo Algorithm**[^tomasulo]으로, 오늘날의 ROB·Reservation Station·Renaming Table까지 곧장 이어집니다.
- **VLIW (또는 Reconfigurable) Processor**에서는 소프트웨어(컴파일러)가 정적으로 이 효과를 만들어냅니다. 대표적인 컴파일러 알고리즘이 1994년의 Iterative Modulo Scheduling입니다.

[^tomasulo]: 1967년 IBM System/360 Model 91의 부동소수점 유닛을 위해 Robert Tomasulo가 고안한 동적 스케줄링 알고리즘. Reservation Station과 Register Renaming이라는 아이디어가 모두 여기서 출발했습니다.
...
- **단점**
  ...
  - 분기를 줄이기 위해 **Predication**[^predication] 같은 기법을 도입할 수 있지만, 결국 컴파일러 복잡도가 그만큼 올라갑니다.
  - 하드웨어가 단순한 만큼, 코드가 충분한 병렬성을 갖지 못한 경우에는 하드웨어가 그것을 보완해 줄 방법이 없습니다. 프로그램을 다시 짜거나 더 강력한 컴파일러를 쓰는 수밖에 없습니다.

[^predication]: 분기 대신 각 명령어에 조건(predicate)을 붙여 두고, 조건이 거짓이면 그 명령어의 결과를 버리는 기법. Control flow를 data flow로 바꿔 분기 없는 직선 코드를 만들어 주므로 SW Pipelining을 적용하기 쉬워집니다.
...
> 우리가 흔히 말하는 Integer 프로그램은 결국 C/C++ 같은 기존 언어로 짜여 있고, VLIW 프로세서가 그런 프로그램을 타깃하려면 그 언어들을 그대로 지원해야 합니다. 따라서 "VLIW 전용 언어"가 따로 있다기보다는, 순차 코드(C/C++ 등)를 받아 병렬 파이프라인 형태로 변환하는 컴파일러 기법이 핵심입니다.

> **Q. VLIW 구조에 잘 맞지 않는 코드는 버블이 발생할 텐데, 별도로 처리해 주는 방법이 있나요?**
> 안타깝게도 하드웨어 단에서 보완해 줄 방법은 거의 없습니다. VLIW는 애초에 하드웨어를 단순하게 만들기 위해 도입되는 구조라, 코드가 충분한 병렬성을 제공하지 못하면 그대로 손실로 이어집니다. 정공법은 프로그램을 다시 짜거나 더 고도화된 컴파일러를 쓰는 것이고, 특히 루프 안 분기가 문제일 때는 Predication으로 분기를 평탄화해 SW Pipelining을 적용할 수 있도록 ISA가 보조 기법을 함께 제공하기도 합니다.

> **Q. TPU나 Tensor Core의 Systolic Array 같은 하드웨어 블록 프로그램에는 VLIW가 유리한가요?**
> Systolic Array[^systolic-array]는 VLIW와는 꽤 다른 형태의 가속기로 보는 편이 정확합니다. 더 극단적으로 프로그래머빌리티 측면의 제약이 크지만, 하드웨어 효율 자체는 훨씬 높습니다. 가지고 있는 프로그래밍 모델 자체가 VLIW와 다르기 때문에, "VLIW가 유리하냐 아니냐"의 문제라기보다는 별도의 갈래로 이해하는 것이 맞습니다.

[^systolic-array]: 연산 유닛(PE)들을 2차원 격자로 배열하고, 데이터가 심장 박동(systole)처럼 매 cycle 이웃 유닛으로 흘러가며 부분 결과가 누적되는 구조. 행렬곱에 특화된 형태로, TPU의 MXU가 대표적인 구현입니다.
...
"갈아 끼울 Warp이 충분히 많다"는 전제는 공짜가 아닙니다. SM이 동시에 들고 있을 수 있는 in-flight Warp의 수를 **Occupancy**라고 부르는데, 이 수는 하드웨어 자원에 의해 뚜렷하게 제한됩니다. H100 기준으로 한 SM은 최대 64개의 resident Warp을 스케줄링할 수 있지만, 그 Warp들이 나눠 써야 하는 Register File은 SM당 256KB(32-bit 레지스터 64K개)뿐입니다. 극단적으로 한 스레드가 상한(약 256개)까지 레지스터를 쓰면, 64K ÷ (32 threads × 256 registers) = 8, 즉 SM에 Warp을 8개밖에 올리지 못합니다. Shared Memory 역시 같은 SM에 올라온 ThreadBlock들이 나눠 쓰는 한정 자원입니다.

결국 Occupancy는 **Latency Hiding의 예산**입니다. 커널이 스레드당 레지스터나 Shared Memory를 많이 쓸수록 갈아 끼울 Warp 후보가 줄어들고, 메모리 Latency를 가려 줄 여력도 함께 줄어듭니다. 반대로 Warp을 많이 올리려고 스레드당 자원을 줄이면 이번에는 스레드 하나가 들고 있을 수 있는 데이터가 작아져 연산의 효율이 떨어집니다. CUDA 커널 최적화에서 늘 등장하는 "Occupancy 튜닝"이 바로 이 트레이드오프를 다루는 작업이고, 이 예산이 바닥나는 지점이 뒤에서 다룰 "SW 주도 Latency Hiding으로의 수렴"의 출발점이 됩니다.

> 수치 출처: [How to Think About GPUs — JAX Scaling Book](https://jax-ml.github.io/scaling-book/gpus/)
...
물론 한 가지 단서를 달아 둘 필요가 있습니다. 우리가 지금 이야기하는 GPU의 일반적 특성은 사실 2000년대에서 2010년대 사이에 그래픽스 워크로드를 기준으로 정의된 모습에 가깝습니다. 최근의 GPU는 ML 워크로드의 특성에 맞춰 빠르게 변모하고 있고, 내부 구조가 모두 공개되지는 않기 때문에 위에서 정리한 특성들이 어디까지 그대로 유지되는지는 외부에서 정확히 단정하기 어렵습니다. 특히 *"All About Hiding Latency"* 라는 측면에서는 현재 세대에서 적지 않은 부분이 다르게 설계되고 있을 가능성도 큽니다. 실제로 어떤 부분이 어떻게 달라졌는지는 강의 마지막의 **수렴** 절에서 Hopper/Blackwell 세대의 SOTA 커널을 통해 구체적으로 확인합니다.
...
이 격차를 실제 수치로 확인해 보면 다음과 같습니다. H100의 메모리 계층을 위(빠르고 작음)에서 아래(느리고 큼)로 내려가며 정리한 표입니다.

| 계층 | 용량 | 비고 |
|---|---|---|
| Register File | 256KB / SM | 스레드들이 나눠 씀 (Occupancy의 상한) |
| Shared Memory (SMEM) | ~228KB / SM | L1과 통합, ThreadBlock 단위로 할당 |
| L2 Cache | ~50MB | 모든 SM이 공유, 실측 ~5.5TB/s |
| HBM3 (GMEM) | 80GB | 3.35TB/s |

여기서 6주차에 다룬 Roofline Analysis를 다시 소환해 볼 수 있습니다. H100의 bf16 연산 성능은 990 TFLOPs/s이고 HBM Bandwidth는 3.35TB/s이므로, 그 비율(터닝 포인트에 해당하는 critical arithmetic intensity)은 약 295입니다. 메모리에서 값 하나를 읽어 올 때마다 대략 300번의 연산을 해 줘야 연산기가 놀지 않는다는 뜻입니다. 흥미로운 것은 B200에서도 이 비율이 2.25 PFLOPs/s ÷ 8TB/s ≈ 280으로 거의 그대로 유지된다는 점입니다. 세대가 바뀌어도 "연산은 남고 메모리가 모자라는" 구조 자체는 변하지 않고, 이 임계값을 넘기기 위해 batch와 tile을 계속 키워야 하는 게임이 이어지고 있는 셈입니다.
...
이 갈래의 가장 잘 알려진 실물 사례가 Google TPU입니다. TPU에는 GPU식 Multithreading이 아예 없습니다. 대신 거대한 행렬 연산 유닛(MXU) 옆에 VMEM이라는 128MB짜리 온칩 메모리를 두고, 어떤 데이터를 언제 VMEM에 올릴지를 컴파일러(XLA)가 전부 정적으로 스케줄링합니다. H100이 SM당 0.5MB 남짓의 SMEM+Register를 하드웨어 스케줄링으로 굴리는 것과 달리, 훨씬 큰 스크래치패드를 소프트웨어가 직접 계획해서 쓰는 구조입니다. 바로 아래에서 설명할 NPU의 공통 구조(Scratchpad + DMA + 정적 실행 계획)와 정확히 같은 문법입니다.
...
- **Scratchpad에 값을 넣고 빼는 모든 동작이 DMA로 명시 프로그래밍** - 위 그림 좌측의 L1 DMA / L2 DMA stack 이 그것입니다. 이 DMA[^dma] 들이 단순히 하드웨어 디테일로 숨겨져 있는 것이 아니라, 소프트웨어가 매 순간 어느 DMA로 어떤 영역을 옮길지 명령으로 직접 적어 주는 구조입니다.

[^dma]: Direct Memory Access. 연산 유닛을 거치지 않고 메모리 영역 사이의 데이터 전송을 전담하는 엔진. CPU/GPU에서는 대부분 하드웨어와 드라이버 뒤에 숨겨져 있지만, NPU에서는 소프트웨어가 직접 명령을 발행하는 1급 프로그래밍 대상입니다.
...
이 구조에서 **Command Processor**가 소프트웨어가 미리 만들어 놓은 실행 계획을 받아 각 하드웨어 유닛에 분배하고, Sync Network를 통해 서브태스크 간 순서 관계를 유지합니다. 그래서 NPU에서 "프로그래밍한다"는 것은 곧 이 Command Stream을 어떻게 짤 것인가의 문제가 됩니다.
...
## 수렴: SOTA GPU 커널은 이미 NPU처럼 프로그래밍된다

앞서 GPU 절에서 *"All About Hiding Latency라는 측면에서는 현재 세대에서 적지 않은 부분이 다르게 설계되고 있을 가능성이 크다"* 는 단서를 달아 두었습니다. 이 절에서 그 단서의 실체를 확인합니다. 결론부터 말하면, Hopper(H100) 이후 GPU에서 성능의 최전선에 있는 커널들은 더 이상 "Warp을 잔뜩 띄워 두면 하드웨어가 알아서 숨겨 준다"는 고전적 방식만으로 동작하지 않습니다. 오히려 방금 NPU 절에서 본 것과 같은 문법(스크래치패드, DMA, 정적으로 짜인 파이프라인)으로 프로그래밍됩니다.

### 왜 고전적 Latency Hiding만으로는 부족해졌는가

출발점은 Tensor Core입니다. Tensor Core는 일반 CUDA core보다 10배 이상 높은 연산 처리량을 제공하는데, 이 처리량을 꽉 채우려면 매 cycle 대량의 tile 데이터를 SMEM과 Register에 공급해 줘야 합니다. 그런데 바로 여기서 앞의 Occupancy 절에서 본 트레이드오프에 부딪힙니다. Arithmetic intensity를 높이려면 output tile을 키워야 하고, tile을 키우면 스레드당 레지스터·SMEM 사용량이 급증해 SM에 올릴 수 있는 Warp 수가 뚝 떨어집니다. 갈아 끼울 Warp이 몇 개 남지 않으니, "다른 Warp으로 전환해서 숨긴다"는 HW 주도 Latency Hiding의 전제 자체가 무너지는 것입니다.

즉 ML 워크로드의 한복판(대형 matmul)에서, GPU는 자신의 고전적 해법을 쓸 수 없는 지점에 도달했습니다. NVIDIA가 내놓은 답은 흥미롭게도 하드웨어를 더 똑똑하게 만드는 것이 아니라, NPU가 갔던 길을 따라가는 것이었습니다.

### Hopper의 답: TMA, Warp Specialization, SW 파이프라인

Hopper 세대에서 추가된 장치들을 NPU의 어휘로 번역해 보면 대응 관계가 놀랄 만큼 정확합니다.

- **TMA (Tensor Memory Accelerator)** - GMEM ↔ SMEM 사이의 벌크 데이터 이동을 전담하는 비동기 복사 엔진입니다. 연산 유닛과 독립적으로 동작하고, 소프트웨어가 "어느 영역을 언제 옮길지"를 명시적으로 지시합니다. 이름만 다를 뿐 NPU의 **DMA**와 같은 역할입니다.
- **프로그래머가 직접 관리하는 SMEM** - SOTA matmul 커널에서 SMEM은 더 이상 하드웨어가 알아서 채워 주는 캐시처럼 쓰이지 않습니다. 몇 개의 버퍼 슬롯을 circular queue로 잡아 두고, 어떤 tile이 어느 슬롯에 언제 들어오고 나갈지를 코드가 직접 결정합니다. 사실상 **Scratchpad**입니다.
- **Warp Specialization** - Warp들을 128개 스레드 단위의 warp-group으로 묶고, 역할을 정적으로 분담시킵니다. **Producer** warp-group은 TMA에 로드 명령을 발행해 데이터를 계속 나르고, **Consumer** warp-group들은 Tensor Core 연산(`wgmma`)만 수행하며, 둘 사이는 barrier로 동기화된 큐로 연결됩니다. "누가 무엇을 담당한다"가 컴파일 시점에 정해진 정적 파이프라인, 즉 앞에서 본 VLIW의 Modulo Scheduling과 같은 발상의 **SW Pipelining**입니다.

### Blackwell의 답: TMEM과 tcgen05

Hopper에서 프로그래머가 소프트웨어로 짜 넣어야 했던 이 구조를, Blackwell(B200)은 하드웨어 차원에서 직접 지원하기 시작합니다. 두 가지 변화가 핵심입니다.

첫째, **TMEM (Tensor Memory)** 이라는 새로운 온칩 메모리가 추가되었습니다. 128개 lane × 최대 512 column, 셀당 32-bit 크기의 2차원 메모리인데, 용도는 MMA의 누적(accumulator) 값을 Register File에서 빼내 따로 담아 두는 것입니다. 큰 tile의 accumulator가 레지스터를 통째로 잡아먹던 문제, 즉 앞에서 본 Occupancy 예산 문제를 하드웨어가 공식적으로 인정하고 전용 메모리로 분리해 준 셈입니다. 더 흥미로운 것은 관리 방식입니다. TMEM은 캐시처럼 하드웨어가 알아서 채워 주는 메모리가 아니라, 커널이 `tcgen05.alloc`으로 명시적으로 할당하고 다 쓰면 명시적으로 해제해야 하는 메모리입니다. 읽고 쓰는 것도 비동기 명령(`tcgen05.ld`/`st`)을 발행한 뒤 완료를 기다리는 식입니다. NPU의 어휘로 번역하면, SMEM에 이어 **스크래치패드가 하나 더 생긴 것**입니다.

둘째, 5세대 Tensor Core 명령어 **`tcgen05.mma`** 는 발행 방식 자체가 바뀌었습니다. Hopper의 `wgmma`는 그래도 warp-group의 128개 스레드가 함께 참여하는 형태였지만, `tcgen05.mma`는 **선출된 스레드 하나가 명령을 발행하면 하드웨어가 tile 전체의 MMA를 알아서 수행**하고, 완료되면 mbarrier[^mbarrier]로 신호를 보내 주는 완전 비동기 동작입니다. SMEM에 있는 입력 tile들의 위치와 형상은 descriptor로 기술해서 넘깁니다. 이 시점에서 Tensor Core는 더 이상 "스레드들이 나눠 밟는 SIMD 연산 유닛"이 아니라, TMA와 대칭을 이루는 **독립된 연산 엔진**입니다. 로드 엔진(TMA)에 "이 영역 옮겨 놔"라고 명령을 발행하듯, 연산 엔진(Tensor Core)에 "이 tile 곱해 놔"라고 명령을 발행하고 완료 신호를 기다리는 구조가 된 것입니다. 여기에 `cta_group::2` 모드는 이웃한 두 CTA[^cta]를 짝지어 하나의 더 큰 output tile을 협력 생산하게 하는, SM 경계를 넘는 정적 협력까지 추가합니다.

[^mbarrier]: Shared Memory에 존재하는 비동기 barrier 객체. TMA 복사나 tcgen05 연산 같은 비동기 작업이 끝나면 여기에 완료 신호를 남기고, 스레드는 그 신호의 도착을 기다리는 방식으로 동기화합니다.

[^cta]: Cooperative Thread Array. CUDA의 ThreadBlock을 하드웨어(PTX) 수준에서 부르는 이름입니다.

<img src="/images/07/blackwell_tcgen05_tmem.png" alt="Blackwell tcgen05 MMA의 데이터 경로: Tensor Core가 TMEM에서 A를, SMEM에서 B를 읽어 결과 C를 TMEM에 누적하고, Register File은 이 경로에서 완전히 비켜나 있다" class="h-72 mx-auto" />

위 그림이 이 데이터 경로를 보여줍니다. Tensor Core는 입력 tile을 TMEM(A)과 SMEM(B)에서 읽고 accumulator(C)를 TMEM에 쓰는데, Register File은 이 경로에서 완전히 비켜나 있습니다. 스레드의 레지스터는 이제 MMA의 통로가 아니라, 명령을 발행하고 결과를 마지막에 회수하는 데만 쓰입니다.

> 그림 출처: [AI Performance Engineering — cfregly](https://github.com/cfregly/ai-performance-engineering) (Apache License 2.0)

이렇게 완성된 Blackwell의 GEMM 파이프라인을 한 장으로 그리면 이렇습니다. TMA 엔진이 tile *k+1*을 GMEM에서 SMEM으로 나르는 동안, Tensor Core는 tile *k*를 계산해 TMEM에 누적하고, epilogue warp-group은 tile *k-1*의 결과를 TMEM에서 레지스터로 읽어 GMEM에 써 냅니다.

<ThemeImage
    lightSrc="/images/07/blackwell_gemm_pipeline_light.svg"
    darkSrc="/images/07/blackwell_gemm_pipeline_dark.svg"
    alt="Blackwell GEMM 3단 파이프라인: TMA가 tile k+1을 나르는 동안 Tensor Core가 tile k를 계산하고 Epilogue warp-group이 tile k-1을 GMEM에 써 내는 중첩 구조"
    class="h-64 mx-auto"
/>

이 3단 파이프라인에서 SIMT 스레드들이 실제로 하는 일은 연산이 아니라 **오케스트레이션**(명령 발행, 버퍼 슬롯 관리, mbarrier 동기화)입니다. 연산 유닛(Tensor Core), 스크래치패드(SMEM/TMEM), DMA(TMA), 동기화 네트워크(mbarrier), 그리고 이들을 지휘하는 소프트웨어. 앞 절에서 본 NPU 아키텍처 다이어그램의 구성 요소가 GPU 안에 그대로 들어앉아 있습니다. "수천 개 스레드가 각자 조금씩"이라는 SIMT의 원형에서, "소수의 역할 분담된 유닛에 큰 작업을 명시적으로 발행"하는 NPU식 그림으로 ISA 수준에서 이동한 것입니다.

> Blackwell 관련 내용은 [Modern GPU Programming for MLSys](https://mlc.ai/modern-gpu-programming-for-mlsys/chapter_background/index.html)의 [tcgen05.mma](https://mlc.ai/modern-gpu-programming-for-mlsys/chapter_tensor_cores/index.html)·[TMEM](https://mlc.ai/modern-gpu-programming-for-mlsys/chapter_tmem/index.html) 챕터를 참고했습니다.

### 숫자로 보는 효과: H100 matmul 워크로그

이 전환이 성능에서 얼마나 결정적인지는 Aleksa Gordić이 정리한 H100 bf16 matmul 커널 최적화 워크로그가 잘 보여줍니다. 각 단계가 어떤 Latency를 숨겨서 얼마를 얻었는지를 따라가 보면 됩니다.

| 단계 | 핵심 아이디어 | TFLOP/s |
|---|---|---|
| Warp-tiling baseline | CUDA core만 사용한 고전적 tiling | 32 |
| Tensor Core + TMA | 연산은 TC로, 로드는 비동기 DMA로 | 317 |
| Output tile 확대 | Arithmetic intensity 증가 | 423 |
| SW Pipelining | Producer/Consumer로 로드와 연산을 중첩 | 498 |
| Tile 128×256 + Consumer 2개 | 더 큰 tile을 warp-group 2개가 분담 | 610 |
| Persistent kernel | Store latency를 다음 tile 연산으로 숨김 | 660 |
| PTX barrier 최적화 | 동기화 비용 절감 | 704 |
| Cluster + TMA multicast | SM 간 중복 로드 제거 | 734 |
| 마이크로 최적화 | | 747 |
| TMA async store | Store도 비동기로 | 758 |
| Hilbert curve 스케줄링 | L2 locality 개선 | 764 |

최종 커널은 cuBLAS 대비 약 107% 성능에 도달합니다. 주목할 점은 이득의 원천입니다. 32 → 764로 가는 여정에서 "Warp을 더 많이 띄운" 단계는 하나도 없습니다. 전부 **데이터 이동을 명시적으로 계획하고, 연산과 겹치도록 정적 파이프라인을 짜는** 방향입니다. 이 커널의 SM 위에는 소수의 warp-group만 떠 있고, 각각의 역할은 컴파일 시점에 이미 정해져 있습니다. Occupancy 기반 Latency Hiding의 반대편 극단에 와 있는 셈입니다.

> 출처: [Inside NVIDIA GPUs: Anatomy of high performance matmul kernels — Aleksa Gordić](https://www.aleksagordic.com/blog/matmul), [Modern GPU Programming for MLSys — GPU Execution Model](https://mlc.ai/modern-gpu-programming-for-mlsys/chapter_background/index.html)

### GPU : NPU는 이분법이 아니라 스펙트럼이다

이렇게 놓고 보면 OOO : VLIW = GPU : NPU 비례식은 두 진영의 출발점을 설명하는 틀이지, 오늘의 현실을 가르는 벽이 아닙니다. ML 워크로드가 지배적인 수요가 되면서 GPU는 꾸준히 SW 주도 쪽으로 이동해 왔습니다. TMA는 DMA가 되었고, SMEM은 Scratchpad처럼 쓰이며, warp specialization 파이프라인은 컴파일 시점에 짜인 실행 계획이라는 점에서 Command Stream과 역할이 겹칩니다. CUDA라는 껍데기는 유지되지만, 그 위에서 실제로 짜는 SOTA 커널의 모습은 NPU 프로그래밍과 점점 닮아가고 있습니다.

강의 첫 절의 질문으로 돌아가면, GPU와 NPU는 서로 다른 종이 아니라 "하드웨어 디테일을 어디까지 소프트웨어에 노출할 것인가"라는 같은 질문에 대한 두 답이고, 현실의 하드웨어는 그 사이 어딘가로 수렴하는 중입니다. 그리고 그 수렴을 이끄는 힘이 바로 이 강의 전체를 관통해 온 Latency Hiding입니다. 숨겨야 할 Latency가 커지고 연산 유닛이 빨라질수록, 그 계획을 하드웨어의 즉흥에 맡기기보다 소프트웨어가 미리 짜 두는 쪽이 유리해지기 때문입니다.
...
다만 이 이분법은 출발점의 구도이고, 현실은 수렴 중입니다. Tensor Core의 처리량을 채우기 위해 큰 tile이 필요해지자 Occupancy 기반의 HW 주도 Latency Hiding은 예산이 바닥났고, Hopper/Blackwell 세대의 SOTA 커널은 TMA(=DMA), 프로그래머가 직접 관리하는 SMEM(=Scratchpad), Warp Specialization(=정적 SW 파이프라인)이라는 NPU의 문법으로 짜입니다. 결국 GPU와 NPU는 "하드웨어 디테일을 어디까지 소프트웨어에 노출할 것인가"라는 프로세서 설계의 핵심 질문에 대한 두 답이고, Latency Hiding의 부담이 커질수록 두 답은 서로를 향해 다가가고 있습니다.
