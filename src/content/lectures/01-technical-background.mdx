...
ML Framework은 서로 다른 뿌리를 가진 네 가지 기술적 흐름 위에서 만들어졌습니다. Heterogeneous computing, Supercomputing, ML compiler, 그리고 Numerical computing을 하나씩 살펴봅니다.
...
- **슈퍼컴퓨팅**은 하나의 거대한 계산을 여러 컴퓨터에서 나누어 처리하며, 모든 작업이 연결되어 있습니다. 따라서 작업 중 하나라도 오류가 발생하면 전체 계산이 망가질 수 있어 "Fault"를 어떻게 처리하느냐가 관건입니다.
...
PyTorch 1과 2는 모두 100% eager 모드를 지원합니다. PyTorch 1에서는 `torch.jit.trace`를 이용해 trace를 뽑아 그래프 모드를 수행했고, PyTorch 2에서는 이를 더 범용적으로 사용할 수 있는 `torch.compile`을 도입했습니다. 특히 trace를 뽑아내는 기능은 Dynamo라는 기술을 통해 더 체계적으로 대응하고 있습니다.
