...
슈퍼컴퓨팅은 엄청나게 복잡한 계산을 처리하기 위한 고성능 컴퓨팅 기술입니다. 최근에는 머신러닝이 대표적인 예시로 떠오르고 있지만, 20년 전만 해도 주로 물리학 시뮬레이션, 천문학, 기후 모델링, 로켓 개발 등 과학 연구와 시뮬레이션 분야에서 사용되었습니다.
...
- 각 HW vendor들은 자사 하드웨어에 최적화된 BLAS 구현을 출시합니다:
  - Nvidia: **cuBLAS** (2017, closed-source), **CUTLASS** (2018, 오픈소스)
...
Data scientist들은 복잡한 수학 함수를 직접 구현하기보다는, 잘 만들어진 도구나 라이브러리를 활용하는 것을 선호합니다:
...
흥미로운 점은, NumPy가 PyTorch의 개발에 영향을 준 뒤, 최근에는 NumPy 창시자의 회사가 PyTorch 2.0에 많은 기여를 하고 있다는 것입니다. 이제 PyTorch는 NumPy 코드를 네이티브로 지원하며, NumPy 코드를 수정 없이 작성하면 Dynamo와 Inductor를 통해 바이트코드로 동적 변환 후 CUDA 코드로 컴파일하여 GPU에서 실행할 수 있습니다.
...
Eager mode와 graph mode를 명확히 구분할 수 있지만, 실제로는 두 가지가 동시에 작동하는 경우가 많습니다. 예를 들어, forward 단계에서는 eager mode로 계산이 진행되지만, autograd를 켜면 backward 단계에서는 gradient 계산을 위한 graph가 생성됩니다. 이처럼 forward에서는 eager mode를, backward에서는 graph mode가 작동합니다.
...
PyTorch 2.0은 PyTorch 1.0의 특성을 거의 모두 계승하면서, 앞서 다룬 여러 기술적 배경을 종합하여 구현된 프레임워크입니다.
...
- Integration with **compute libraries / ML compiler** — Eager mode에서는 컴퓨팅 라이브러리, Graph mode에서는 ML 컴파일러와 통합
- Three language layers: **Python → C++ → kernel language** (예: CUDA, Triton) — 멀티 랭귀지 솔루션
...
Triton은 단순한 Python API가 아닌, Python 신택스를 사용한 도메인 특화 언어(DSL)로 보는 것이 더 정확합니다. PyTorch 2.0에서 기본으로 채택한 커널 프로그래밍 언어로 이해할 수 있습니다.
