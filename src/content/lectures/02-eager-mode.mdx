...
Call stack이 55단계나 되니 매우 복잡한 구조처럼 보이지만, 상당수는 자동 생성된 wrapper와 반복되는 dispatcher logic입니다. 이를 구분하고 나면 개발자가 직접 작성하는 핵심 부분은 `matmul`이 input shape에 따라 연산을 선택하는 코드(#25-26)와 CUDA에서 `mm` operator를 실행하는 device-specific 구현(#0-4)으로 정리할 수 있습니다.
