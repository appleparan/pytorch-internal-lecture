...
| **Global set** | 항상 고려되는 dispatch key (참고: Autograd는 이후 global set에서 tensor로 이동됨) |
...
오랫동안 PyTorch는 `python setup.py develop` / `python setup.py install`로 빌드했지만, 이 방식은 공식적으로 deprecated 되었습니다. 이제 PyTorch는 **scikit-build-core**를 통해 표준 **PEP 517** 인터페이스(`pyproject.toml`)로 빌드되며, `setup.py`는 빌드에 전혀 관여하지 않는 forwarding stub만 남았습니다.
...
- `spin`은 scientific-python 프로젝트들이 쓰는 developer CLI로, `pip install --group dev` 시 함께 설치됨
- 환경변수를 통한 빌드 커스터마이즈(`DEBUG=1`, `USE_CUDA=0`, `MAX_JOBS=...` 등)는 대체 명령에서도 그대로 동작
...
`_subclasses` `_vendor` `amp` `ao` **`autograd`** `backends` `compiler` `contrib`
...
`special` `testing` **`torch`** `utils` **`xpu`**
