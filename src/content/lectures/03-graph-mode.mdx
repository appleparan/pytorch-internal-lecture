...
- PyTorch의 역사와 2.0의 변화 - eager mode 중심이던 PyTorch에 graph mode가 더해지기까지의 배경
- Graph capturing - graph mode의 근간이 되는 tracing
    - 컴퓨터 구조 분야의 고전 사례로 정리하는 tracing 개념: out-of-order execution, trace cache, partial evaluation
    - PyTorch 1.0의 시도와 한계: `jit.trace`(입력 의존성 검증 부재), `jit.script`(Python subset 처리 범위의 한계)
    - 2.0의 해법 Dynamo: CPython bytecode 수준의 trace 생성, guard를 통한 안전성 검증, graph break를 통한 trace 불가 구간 자동 분할
- IR lowering - Dynamo가 만든 FxGraph가 Torch IR → Aten/Prims IR → Inductor Loop-Level IR로 낮아지는 과정
