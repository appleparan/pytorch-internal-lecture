...
- Tracing할 이득이 없거나 불가능한 코드(내장 함수, stdlib, torch 내부 구현 등) → skip 목록에 따라 원래 방식으로 실행
...
1. Guard를 검사하여 이전 trace의 전제(static input)가 여전히 유효한지 확인
2. 통과하면 cache된 최적화 코드를 그대로 실행 — compilation overhead 없음
3. 실패하면 recompile하여 새로운 trace를 생성
